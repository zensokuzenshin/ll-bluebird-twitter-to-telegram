...
try:
    ...
    from anthropic import APIStatusError as AnthropicAPIStatusError, APIError as AnthropicAPIError
...
try:
    ...
    from openai import APIStatusError as OpenAIAPIStatusError, APIError as OpenAIAPIError
...
class AnthropicProvider(LLMProvider):
    ...
    def __init__(self, model_name: str):
        ...
        if not self.api_key:
            raise TranslationError("No Anthropic API key provided. Set ANTHROPIC_API_KEY environment variable.")
    ...
    async def translate(
        ...
        if not ANTHROPIC_AVAILABLE:
            raise TranslationError("Anthropic package is not installed. Install with 'pip install anthropic'.")
        ...
        while True:
            try:
                logger.info(f"Translating text using Anthropic model: {self.model_name}")
                ...
                if is_rate_limit and retry_count < max_retries:
                    ...
                    logger.error(f"Anthropic translation failed after {retry_count} retries due to rate limits: {str(e)}")
                    raise RateLimitedError(f"Anthropic translation failed due to rate limits: {str(e)}")
                else:
                    # Not a rate limit error
                    logger.error(f"Anthropic API error: {str(e)}")
                    raise TranslationError(f"Anthropic translation failed with API error: {str(e)}")
            
            except Exception as e:
                # Not a rate limit error, don't retry
                logger.error(f"Anthropic translation failed with non-retryable error: {str(e)}")
...
class OpenAIProvider(LLMProvider):
    ...
    def __init__(self, model_name: str):
        ...
        if not self.api_key:
            raise TranslationError("No OpenAI API key provided. Set OPENAI_API_KEY environment variable.")
    ...
    async def translate(
        ...
        if not OPENAI_AVAILABLE:
            raise TranslationError("OpenAI package is not installed. Install with 'pip install openai'.")
        ...
        while True:
            try:
                ...
                if is_rate_limit and retry_count < max_retries:
                    ...
                    # We've exhausted retries for rate limits
                    logger.error(f"OpenAI translation failed after {retry_count} retries due to rate limits: {str(e)}")
                    raise RateLimitedError(f"OpenAI translation failed due to rate limits: {str(e)}")
                else:
                    # Not a rate limit error
                    logger.error(f"OpenAI API error: {str(e)}")
                    raise TranslationError(f"OpenAI translation failed with API error: {str(e)}")
            
            except Exception as e:
                # Not a rate limit error, don't retry
                logger.error(f"OpenAI translation failed with non-retryable error: {str(e)}")
    ...
    for model_spec in models_to_try:
        try:
            ...
            if ":" not in model_spec:
                logger.warning(f"Invalid model specification '{model_spec}'. Must be in 'provider:model' format.")
                continue
            ...
            try:
                ...
                logger.warning(f"Rate limited with {provider_name}:{model_name}. Trying next provider.")
                ...
                logger.warning(f"Translation failed with {provider_name}:{model_name}: {str(e)}. Trying next provider.")
                errors.append(f"{provider_name}:{model_name} - Error: {str(e)}")
                continue
                
        except Exception as e:
            # General errors with this provider, try the next one
            logger.warning(f"Error with provider {model_spec}: {str(e)}. Trying next provider.")
